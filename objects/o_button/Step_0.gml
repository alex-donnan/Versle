...
if (active) {
	...
	if ((LEFT_RELEASE && collision_point(MOUSE_X, MOUSE_Y, self, 1, false))
		|| (utility == 0 && keyboard_check_pressed(ord(string_upper(letter))))
		|| (utility == 1 && keyboard_check_pressed(vk_backspace) && letter == "Back")
		|| (utility == 1 && keyboard_check_pressed(vk_enter) && letter == "Enter")) {
		with (o_data) {
			if (other.letter == "Back" || keyboard_check_pressed(vk_backspace)) {
				remove_letter();
			} else if (other.letter == "Enter" || keyboard_check_pressed(vk_enter)) {
				check_guess();
			} else {
